...
(function($) {
	...
	var methods = {
			...
			for (var i = 0; i < rules.length; i++) {
				...
				switch (rules[i]) {
					...
					case "groupRequired" :
						...
						;
						errorMsg = methods._groupRequired(field, rules, i,
								options);
						if (errorMsg)
							required = true;
					...
					case "past" :
						errorMsg = methods
								._past(form, field, rules, i, options);
						break;
					case "future" :
						errorMsg = methods._future(form, field, rules, i,
								options);
						break;
					case "dateRange" :
						var classGroup = "[" + options.validateAttribute + "*="
					...
					case "equals" :
						...
						break;
					case "equalsNewPwd" :
						errorMsg = methods._equalsNewPwd(field, rules, i, options);
						break;
					case "funcCall" :
					...
					case "creditCard" :
						errorMsg = methods
								._creditCard(field, rules, i, options);
						break;
		...
		_equalsNewPwd : function(field, rules, i, options) {
			var equalsField = rules[i + 1];

			if (field.val() != $("#" + equalsField).val())
				return options.allrules.equalsNewPwd.alertText;
		},
		/**
		...
		_calculatePosition : function(field, promptElmt, options) {
			...
			// ----------------------------------------------------------------上边距我减去了5---------------------------------------
			var fieldTop = -5 + field.position().top;
